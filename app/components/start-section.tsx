...
import { stats } from "@/lib/api/landing-page-data"
import { CheckCircle2 } from "lucide-react"
import { features } from "@/lib/api/landing-page-data"
    ...
    <section className="py-20 lg:px-16 md:px-8 px-4 bg-[#1A1015] ">
      <div className="container">
          ...
          <div className="space-y-4 grid lg:grid-cols-1 md:grid-cols-2">
            {features.map((feature) => (
              <div key={feature.title} className="flex gap-3">
                ...
                <div>
                  <div className="font-semibold text-lg text-base-light-white">{feature.title}</div>
                  <div className="text-sm text-base-light-white font-extralight">{feature.description}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {stats.map((stat) => (
            <Card key={stat.label} className="py-4 text-center bg-brand-primary-stroke-strong  border-0">
              <div className="text-4xl font-bold mb-1 text-amber-text">{stat.value}</div>
